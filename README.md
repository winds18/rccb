...
- `./.claude/agents/*.md`
- `./.claude/agents/delegate-*.md`（Claude 子代理委派模板）
...
Codex / Gemini / Claude 强化点：

- `Codex` 额外生成审计与调研复核技能：`rccb-audit`、`rccb-research-verify`
- `GEMINI.md` 内置两轮调研、多路径交叉验证、冲突项显式输出的工作流
- `Claude` 额外生成 `delegate-coder` / `delegate-researcher` / `delegate-auditor` / `delegate-scribe` 子代理，默认通过 `rccb ask --async` 做上下文隔离式派单
...
11. 当编排者是 `claude` 时，默认允许使用 Claude `Task`/子代理能力；若任务可拆分、希望并行推进或不想污染主上下文，优先调用 `delegate-*` 子代理异步派单
12. `debug` 不是粘滞状态：上一次即使开过 debug，这一次若未显式指定 `--debug` 或 `RCCB_DEBUG=1`，也不会自动拉起 debug pane
13. 可选开关：
...
如果当前编排者是 `claude`，推荐优先让子代理派单：

```text
delegate-coder        -> opencode
delegate-researcher   -> gemini
delegate-auditor      -> codex
delegate-scribe       -> droid
```
...
- `droid`：文档记录者
- `codex`：代码审计者
- `claude`：编排者；优先通过 `delegate-*` 子代理做异步派单与上下文隔离
