...
{}\n\n\
## 上下文打包规则\n\
- 复杂工程任务在委派时必须带足上下文，尤其是项目路径、目标目录、关键文件路径、受影响模块、当前状态、目标状态和验收标准。\n\
- 如果任务依赖某个错误日志、测试失败、命令输出或工件文件，只传和当前判断有关的片段，并明确它对应的路径或来源。\n\
...
## 上下文打包\n\
- 复杂工程任务必须附带：项目根目录、目标路径、关键文件、当前状态、目标状态、验收口径。\n\
