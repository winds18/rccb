...
`RCCB_PROVIDER_ROLE=orchestrator RCCB_PROVIDER_AGENT=delegate-coder RCCB_ASK_ASYNC_STDOUT=minimal rccb --project-dir . ask --instance default --provider opencode --caller claude --async --timeout-s 900 \"$ARGUMENTS\"`\n\n\
    ...
    if providers.iter().any(|p| p == "claude") && providers.iter().any(|p| p == "gemini") {
        specs.push(RuleFileSpec {
            ...
            contents: build_claude_command_markdown(
                ...
                "使用 RCCB 先把调研任务委派给 `gemini`，要求它做详细、结构化、有逻辑的调研，优先官方/一手来源，并明确标注日期、推导过程与冲突点。\n\
任务内容：$ARGUMENTS\n\n\
...
`rccb --project-dir . inbox --instance default --req-id <req_id> --latest --limit 5`。",
    ...
    if providers.iter().any(|p| p == "claude") && providers.iter().any(|p| p == "codex") {
        specs.push(RuleFileSpec {
            ...
            contents: build_claude_command_markdown(
                "委派代码审计/复核任务给复核执行者",
                "使用 RCCB 把代码审计、风险评估、边界条件检查、回归分析和调研复核任务委派给复核执行者。\n\
任务内容：$ARGUMENTS\n\n\
...
fn should_refresh_legacy_plain_rule(path: &Path, existing: &str) -> bool {
    ...
    if path_str.ends_with(".claude/commands/rccb-code.md")
        && (!normalized.contains("delegate-coder")
            || !normalized.contains("RCCB_PROVIDER_AGENT=delegate-coder"))
    {
        return true;
    }
    ...
    if path_str.contains(".claude/agents/delegate-")
        && (!normalized.contains("tools: ['Bash']")
            || !normalized.contains("RCCB_PROVIDER_ROLE=orchestrator")
            || !normalized.contains("RCCB_PROVIDER_AGENT="))
    {
        return true;
    }
...
fn build_claude_runtime_rule_markdown(
    ...
    if contains_provider(providers, "opencode") {
        delegate_lines.push(format!(
            "- 编码委派：`RCCB_PROVIDER_ROLE=orchestrator RCCB_PROVIDER_AGENT=delegate-coder RCCB_ASK_ASYNC_STDOUT=minimal {base_cmd} ask --instance {effective_instance} --provider opencode --caller claude --async --timeout-s 900 \"<任务>\"`"
        ));
    }
    if contains_provider(providers, "gemini") {
        delegate_lines.push(format!(
...
fn maybe_prepare_tmux_mouse_support() {
    ...
    if let Err(err) = ensure_tmux_mouse_support() {
        eprintln!("警告：tmux mouse 配置自动修正失败：{}", err);
    }
}
...
fn resolve_tmux_config_path() -> Result<PathBuf> {
    if let Some(raw) = env::var("RCCB_TMUX_CONFIG_PATH")
        .ok()
    ...
    let xdg_candidate = env::var("XDG_CONFIG_HOME")
        ...
        .join("tmux")
        .join("tmux.conf");
    let legacy_candidate = home_dir.join(".tmux.conf");
    ...
    if env::var("XDG_CONFIG_HOME").is_ok() || home_dir.join(".config").join("tmux").exists() {
        return Ok(xdg_candidate);
    }
    Ok(legacy_candidate)
}

fn tmux_home_dir() -> Result<PathBuf> {
    env::var("HOME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
        .map(PathBuf::from)
        .ok_or_else(|| anyhow!("无法解析 HOME，无法定位 tmux 配置文件"))
}

fn ensure_tmux_mouse_config_file(path: &Path) -> Result<bool> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .with_context(|| format!("创建 tmux 配置目录失败：{}", parent.display()))?;
    }
    ...
    let existing = match fs::read_to_string(path) {
        ...
        Err(err) => {
            return Err(err).with_context(|| format!("读取 tmux 配置失败：{}", path.display()));
        }
    };

    let (next, changed) = rewrite_tmux_mouse_config_contents(&existing);
    if changed {
        fs::write(path, next.as_bytes())
            .with_context(|| format!("写入 tmux 配置失败：{}", path.display()))?;
    }
    Ok(changed)
}

fn rewrite_tmux_mouse_config_contents(existing: &str) -> (String, bool) {
    let normalized = existing.replace("\r\n", "\n");
    ...
    let mut saw_mouse_on = false;
    let mut last_mouse_idx: Option<usize> = None;
    ...
    for (idx, line) in lines.iter_mut().enumerate() {
        ...
        if !line_contains_tmux_mouse_directive(&tokens) {
            continue;
        }

        last_mouse_idx = Some(idx);
        let current_value = tokens.last().copied().unwrap_or_default();
        if current_value.eq_ignore_ascii_case("on") {
            saw_mouse_on = true;
            continue;
        }
        if current_value.eq_ignore_ascii_case("off") {
            let updated = replace_tmux_mouse_value(line, "on");
            if *line != updated {
                *line = updated;
                changed = true;
            }
    ...
    if !saw_mouse_on {
        let appended = "set -g mouse on".to_string();
        match last_mouse_idx {
            Some(idx) => lines.insert(idx + 1, appended),
            None => {
                if !lines.is_empty() && !lines.last().is_some_and(|v| v.trim().is_empty()) {
...
fn line_contains_tmux_mouse_directive(tokens: &[&str]) -> bool {
    if tokens.len() < 3 {
        return false;
    }
    ...
    tokens
        ...
        .any(|token| token.eq_ignore_ascii_case("mouse"))
}

fn replace_tmux_mouse_value(line: &str, value: &str) -> String {
    let newline = if line.ends_with('\n') { "\n" } else { "" };
    let body = line.trim_end_matches('\n');
    let (statement, comment) = match body.split_once('#') {
        Some((left, right)) => (left.trim_end(), Some(right)),
        None => (body.trim_end(), None),
    };
    let mut tokens = statement.split_whitespace().collect::<Vec<_>>();
...
"#,
    ...
    #[test]
    fn rewrite_tmux_mouse_config_turns_off_into_on() {
        let raw = "set -g status on\nset -g mouse off\n";
        let (next, changed) = super::rewrite_tmux_mouse_config_contents(raw);
        assert!(changed);
        assert!(next.contains("set -g mouse on"));
        assert!(!next.contains("mouse off"));
    }

    #[test]
    fn rewrite_tmux_mouse_config_appends_when_missing() {
        let raw = "set -g status on\n";
        let (next, changed) = super::rewrite_tmux_mouse_config_contents(raw);
        assert!(changed);
        assert!(next.ends_with("set -g mouse on\n"));
        assert!(next.contains("set -g status on"));
    }

    #[test]
    fn rewrite_tmux_mouse_config_keeps_existing_on() {
        let raw = "set -g mouse on\nset -g status on\n";
        let (next, changed) = super::rewrite_tmux_mouse_config_contents(raw);
        assert!(!changed);
        assert_eq!(next, raw);
    }
    ...
    #[test]
    fn ensure_tmux_mouse_config_file_creates_missing_file() {
        let project = std::env::temp_dir().join(format!("rccb-tmux-mouse-{}", now_unix_ms()));
        let path = project.join(".tmux.conf");
        let changed = super::ensure_tmux_mouse_config_file(&path).unwrap();
        assert!(changed);
        let raw = fs::read_to_string(&path).unwrap();
        assert_eq!(raw, "set -g mouse on\n");
        let _ = fs::remove_dir_all(&project);
    }

    #[test]
    fn resolve_tmux_config_path_prefers_existing_xdg_file() {
        let _guard = env_lock().lock().unwrap();
        let root = std::env::temp_dir().join(format!("rccb-tmux-path-{}", now_unix_ms()));
        let home = root.join("home");
        let xdg = root.join("xdg");
        let xdg_file = xdg.join("tmux").join("tmux.conf");
        fs::create_dir_all(xdg_file.parent().unwrap()).unwrap();
        fs::write(&xdg_file, "set -g mouse on\n").unwrap();
        fs::create_dir_all(&home).unwrap();

        let old_home = std::env::var("HOME").ok();
        let old_xdg = std::env::var("XDG_CONFIG_HOME").ok();
        let old_override = std::env::var("RCCB_TMUX_CONFIG_PATH").ok();
        unsafe {
            std::env::set_var("HOME", &home);
            std::env::set_var("XDG_CONFIG_HOME", &xdg);
            std::env::remove_var("RCCB_TMUX_CONFIG_PATH");
        }

        let path = super::resolve_tmux_config_path().unwrap();
        ...
        if let Some(v) = old_override {
            unsafe { std::env::set_var("RCCB_TMUX_CONFIG_PATH", v) };
        } else {
