...
7. pane 复制体验修复
   - 排查 `tmux` 下子任务活跃时，Claude pane 选区复制失真、复制内容与实际选中内容不一致的问题
   - 收口目标：保持鼠标可用的同时，不让运行态注入、OSC52 或 pane feed 干扰用户复制

8. 手动指定执行者优先级彻底压过默认链路
   - 当用户显式指定“调研让 opencode 做”“审计让 droid 做”时，编排者与执行者都必须服从指定
   - 执行者被派到具体任务后默认直接执行，除非任务文本显式要求再委派或多阶段核验；不能因为共享项目规则而误把自己当成编排者

9. 手动取消语义细化
   - 用户中途取消下游任务时，尽量向上游返回 `canceled` / `已取消`，不要笼统折叠成 `timeout`
   - 编排者拿到取消态后应停止追加派单，并用更克制的文案说明“任务被人工终止”

10. provider CLI 异常上报与优雅处理
   - 当执行者 CLI 自身出现异常（如 `api error: premature close`、连接中断、provider 侧崩溃）时，编排者要能稳定感知，而不是误判成普通超时或静默无感
   - 收口目标：把异常分成“可重试 / 已取消 / 明确失败 / 未知异常”几类，并向编排者返回克制、可执行的状态说明

11. tmux pane 复制回归
   - 已定位一处核心回归：当 tmux 原本已经 `mouse on` 时，RCCB 会提前返回，导致复制优先绑定根本没有注入
   - 已补强运行态复制链路：复制优先绑定改为贴近 tmux 默认拖拽流程，并显式接系统剪贴板命令，降低“复制不到 / 复制内容不对”的概率
   - 待用户实测确认：tmux pane 中鼠标拖选是否已恢复为“选中即复制”，以及 cleanup/stop 后绑定恢复是否符合预期

12. `.rccb` 一键清理调试脚本
   - 在 `.rccb` 下提供一个清理脚本，方便开发/调试时一键清理 RCCB 托管生成的 provider 配置、规则、skills 与运行态残留
   - 收口目标：只清理 RCCB 托管内容，不误删用户自定义内容，并能快速把项目恢复到“重新初始化/重新启动即可复现”的状态
...
6. 修复 `tmux` 下子任务运行时的 pane 复制异常，确认鼠标与复制链路可同时稳定工作
7. 修复“手动指定执行者仍被默认 gemini/codex 链路覆盖”的规则漂移，避免简单天气类任务被过度工程化
8. 细化手动取消任务的终态语义，减少上游把人工取消误报成超时
9. 补齐 provider CLI 异常分类与编排者可见性，重点覆盖 Gemini `api error: premature close` 这类可恢复异常
10. 修复 tmux pane 复制完全不可用的回归，并验证在有子任务活跃时仍能稳定复制
11. 提供 `.rccb` 调试清理脚本，方便一键清空托管生成的 provider 配置/skills/运行态残留
